...
function update_graph(njson, clicked_node) {

  console.log("njson nodes before adding:", njson.nodes);
  
  var svgGroup = d3.select("#svgGroup");
  ...
  var new_positionY = get_new_node_posY(clicked_node)[1]
  
  var new_positionX = 0

  var clicked_children = get_new_node_posY(clicked_node)[0]
  //if clicked node has children, get their positionX and move the rest of the nodes
  if (clicked_children.length == 0) {
    new_positionX = parseInt(clicked_node.positionX) + distX + clicked_node.name.length * pixelmultiplier
  }
  else {
    new_positionX = parseInt(njson.nodes[clicked_children[0]].positionX)
    //move all nodes that are below and their ancestors
  }
    /*
  Check if clicked_node has children
  if yes: 
  - get previous sibling's position
  if no:
  - get parents' position
  */
  //var new_positionY = parseInt(clicked_node.positionY)
  
  console.log("new_positionX:",new_positionX);
  console.log("new_positionY:",new_positionY);

  //update node positions in the overall array
  njson.nodes[njson.nodes.length-1].positionX = new_positionX
  njson.nodes[njson.nodes.length-1].positionY = new_positionY

  //console.log("last node:",njson.nodes[njson.nodes.length-1]);

  //add the created node to the list of children of the clicked node
  clicked_node.children.push(njson.nodes.length-1);

  //draw circle
  elemEnter.append('circle')
      .filter(function(d, i) {return i == njson.nodes.length-1})
      .attr("id", function(d, i) {
        console.log(d,i);
      })
      .attr("fill", "lightsteelblue")
      .attr("r", radius)
      .attr("cx", new_positionX)
      .attr("cy", new_positionY)
      .attr("stroke", "steelblue")
      .attr("stroke-width", 2)
      .transition();
  ...
  elemEnter.append('text')
      .filter(function(d, i) {return i == njson.nodes.length-1})
      .attr("dx", new_positionX + 10)
      .attr("dy", new_positionY)
      .attr("class", "labels")
      .text(function(d){
        return d.name;
      })
  ...
  lineData[0].x = parseInt(clicked_node.positionX)
  lineData[0].y = parseInt(clicked_node.positionY)
  
  lineData[1].x = parseInt(new_positionX)
  lineData[1].y = parseInt(new_positionY)

  //console.log("linedata:", lineData)

  //draw edge between clicked node and new node
  elemEnter.append("path")
    .attr("d", linkVertical(lineData))
    .attr("source", clicked_node.nodeid)
    .attr("target", njson.nodes[njson.nodes.length-1].nodeid)
    .attr("class", "edges")
    .attr("stroke", "lightsteelblue")
    .attr("stroke-width", 1)
    .attr("fill", "none");

  console.log("njson nodes after adding:", njson.nodes);
  
  //update variable with node positions in localStorage
  localStorage.setItem("nodes_pos", JSON.stringify(njson.nodes))
}
